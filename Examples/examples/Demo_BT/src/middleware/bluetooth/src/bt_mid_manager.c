...
//			BtStackInitialized();
		...
		{
			printf("!!!ERR: BT_STACK_EVENT_COMMON_STACK_FREE_MEM_SIZE\n");
		}
		break;
		...
		case BT_STACK_EVENT_COMMON_MODE_CHANGE:
			printf("mode - %d, addrs -  %02x:%02x:%02x:%02x:%02x:%02x\n",
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case BT_STACK_EVENT_COMMON_CONNECTION_ABORTED:
			printf("BT_STACK_EVENT_COMMON_CONNECTION_ABORTED\n");
			//BtDevConnectionAborted();
			break;

		case BT_STACK_EVENT_COMMON_PAGE_TIME_OUT:
			printf("BT_STACK_EVENT_COMMON_PAGE_TIME_OUT\n");
...
//			BtDevConnectionLinkLoss(param);
        ...
        case BT_STACK_EVENT_COMMON_LINK_DISCON_LOCAL:
			printf("BT_STACK_EVENT_COMMON_LINK_DISCON_LOCAL\n");
			BTSetAccessMode(BtAccessModeGeneralAccessible);
            break;
...
uint8_t bt_link_index_get(uint8_t *addr)
{
	...
	for(i=0; i<1; i++)
	{
		if(memcmp(addr, btManager.btLinked_env[i].remoteAddr, 6) == 0)
		{
			return i;
		}
	}
...
uint8_t BB_wr_page_to(uint16_t  page_to)
{
	return 0;
}
...
void BB_wr_inq_tx_pwr_lvl(int8_t tx_pwr)
{
}

uint8_t rf_txpwr_inc_app(uint8_t id)
{
	return 0;
}

uint8_t hci_evt_filter_con_add(struct con_set_filter const * filter)
{
	return 0;
}

uint8_t hci_evt_filter_reset(void)
{
	return 0;
}

uint8_t hci_evt_filter_inq_add(struct inq_res_filter const * filter)
{
	return 0;
}

uint16_t gapc_hci_handle_get(uint8_t conidx)
{
	return 0;
}

void host_send_data(uint8_t *data, uint32_t len)
{
