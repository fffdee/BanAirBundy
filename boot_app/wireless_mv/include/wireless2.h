...
#define MVW2_LIB_VERSION	"1.3.43"
...
void wireless2_test_mode_slave_start(wireless_test_mode_xo_cal_result_cb cb);
...
/**
 * @brief  传输包请求,应用层重构
 * @param  StepNum:帧起始packet tx计数，0：unfind Frame sync；1：send packet first ；2：send packet sencond ;
...
/**
 * @brief  Ready Packet长度,应用层重构
 * @param  无
 * @return uint16_t: Ready packet len @byte
 */
uint16_t Wireless_RfTransBufLen();
