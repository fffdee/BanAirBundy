...
static int32_t bg_flash_read_system(uint32_t offset, uint8_t *buffer, uint32_t size)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!buffer || size == 0) return BG_FLASH_ERROR_PARAM;
    if (!is_valid_system_range(offset, size)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(1000)) return BG_FLASH_ERROR_TIMEOUT;
    
    ret = FlashPartition_SystemRead(offset, buffer, size);
    if (ret != FLASH_OK) g_flash_status.flash0.error_count++;
    flash_unlock();
...
static int32_t bg_flash_write_system(uint32_t offset, const uint8_t *data, uint32_t size)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!data || size == 0) return BG_FLASH_ERROR_PARAM;
    if (!is_valid_system_range(offset, size)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(2000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_erase_system_sector(uint32_t offset)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!is_valid_system_range(offset, BG_FLASH_SECTOR_SIZE)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(5000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_read_looper(uint32_t offset, uint8_t *buffer, uint32_t size)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!buffer || size == 0) return BG_FLASH_ERROR_PARAM;
    if (!is_valid_looper_range(offset, size)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(1000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_erase_looper_sector(uint32_t offset)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!is_valid_looper_range(offset, BG_FLASH_SECTOR_SIZE)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(5000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_erase_looper_block(uint32_t offset)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!is_valid_looper_range(offset, BG_FLASH_BLOCK_SIZE)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(10000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_erase_looper_all(void)
{
    ...
    if (!dev || !dev->initialized) return BG_FLASH_ERROR_NOT_INIT;
    
    if (!flash_lock(30000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_read_storage(uint32_t offset, uint8_t *buffer, uint32_t size)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!buffer || size == 0) return BG_FLASH_ERROR_PARAM;
    if (!is_valid_storage_range(offset, size)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(1000)) return BG_FLASH_ERROR_TIMEOUT;
    
    ret = FlashPartition_StorageRead(offset, buffer, size);
    if (ret != FLASH_OK) g_flash_status.flash1.error_count++;
    flash_unlock();
...
static int32_t bg_flash_erase_storage_sector(uint32_t offset)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!is_valid_storage_range(offset, BG_FLASH_SECTOR_SIZE)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(5000)) return BG_FLASH_ERROR_TIMEOUT;
...
static int32_t bg_flash_erase_storage_block(uint32_t offset)
{
    ...
    if (!g_initialized) return BG_FLASH_ERROR_NOT_INIT;
    if (!is_valid_storage_range(offset, BG_FLASH_BLOCK_SIZE)) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(10000)) return BG_FLASH_ERROR_TIMEOUT;
...
{
    if (!status) return BG_FLASH_ERROR_PARAM;
    if (!flash_lock(100)) return BG_FLASH_ERROR_TIMEOUT;
    
    FlashDevice_t *dev0 = FlashDevices_GetSystemFlash();
    FlashDevice_t *dev1 = FlashDevices_GetStorageFlash();
    if (dev0) update_device_status(dev0, &g_flash_status.flash0);
    if (dev1) update_device_status(dev1, &g_flash_status.flash1);
    memcpy(status, &g_flash_status, sizeof(BG_FlashMgrStatus_t));
    flash_unlock();
    return BG_FLASH_OK;
}

static bool bg_flash_is_ready(void) { return g_initialized && g_flash_status.flash0.ready; }
static uint32_t bg_flash_get_looper_free_space(void) { return BG_FLASH_PARTITION_LOOPER_SIZE; }
static uint32_t bg_flash_get_storage_free_space(void) { return BG_FLASH_PARTITION_STORAGE_SIZE; }
