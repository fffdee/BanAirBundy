...
------------------------------------------------------------*/
	...
	/* bootloader starts independently from 0x000000, NOT from a handoff.
	 * Chip_MemInit() MUST configure MPU so SRAM is accessible before
	 * __c_init() copies .data / clears .bss.  Without MPU setup the first
	 * SRAM write triggers a bus fault (PC=0xffff0000). */
	Chip_MemInit();
